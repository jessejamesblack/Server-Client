...
void *mergeThread(void *mergeArgs)
{
    ...
    struct mergeArgs *args = mergeArgs;
    struct Tokenizer **arr1 = args->arr1;
    int size1 = args->size1;
    struct Tokenizer **arr2 = args->arr2;
    int size2 = args->size2;
    char *columnName = args->columnName;
    int returnindex = args->returnIndex;

    struct Tokenizer **arr3 = malloc((size1 + size2) * sizeof(struct Tokenizer *));
    ...
    Returner *pair = malloc(sizeof(*pair));
    pair->array = arr3;
    pair->size = size1 + size2;
    pthread_exit(pair);
}
