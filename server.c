	...
	ClientArrayEntry clientArray[50];
  ...
  while ((new_socket = accept(server_fd, (struct sockaddr *)&address,(socklen_t *)&addrlen)))
	   {
		...
		/*	pthread_mutex_lock(&clientArrayLock);
			// loop to assign a client to a spot in the array	
			while(1){
				if(clientArray[k].openSpotInArray == 0){
					clientArray[k].clientTID = tid;
					clientArray[k].openSpotInArray = 1;
					clientArray[k].socketID = new_socket;
					clientArray[k].clientReadyToJoin = 0;
					clientArray[k].clientID = clientID;
					break;				
				}	
				k++;
				if(k > clientArraySize){
					// all entries are filled so we need to wait on the first one, and put this new client in the first spot, to prevent getting locked up
					pthread_join(clientArray[0].clientTID, NULL);
					clientArray[0].clientTID = tid;
					clientArray[0].openSpotInArray = 1;
					clientArray[0].socketID = new_socket;
					clientArray[0].clientReadyToJoin = 0;
					clientArray[0].clientID = clientID;
					break;				
				}			
			}				
		*/
	...
	/*		// join loop on the clientEntry Array	
			for(j = 0; j < clientArraySize; j++ ){
				if(clientArray[j].clientReadyToJoin == 1){
					pthread_join(clientArray[j].clientTID, NULL);
					clientArray[j].clientReadyToJoin = 0;				
				}			
			}
...
void * clientHandler (void * args){
		...
		//while((c = recv(socketFD, columnName, 1024, 0)) > 0) {}
		//printf("column: %s\n", columnName);
	...
	// sort file
	sortedReturn = sorter(filename, "color");
